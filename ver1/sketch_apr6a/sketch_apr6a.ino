...
int uvsensorpin = 35;   //Pin de SALIDA sensor GYML8511 en el D13 del ESP
int gassensorpin = 34;  //Pin de SALIDA sensor MQ135 en el D14 del ESP
int DHTPIN = 33;        //Pin de SALIDA sensor DHT22 en el D33 del ESP
...
const int pinRedGas=16;
const int pinGreenGas=17;
...
void loop() {
  ...
  float sum=0.0;
  for(int i=0;i<6;i++){
    uvIntensities[i]=(OutVoltage-1.0)/divisores[i];
    sum += uvIntensities[i];
  }
  ...
  float Rs=1000*((5-VoltageGas)/VoltageGas);
  //Hallar Ro
  float CO2=124.75*pow(Rs/718.5,-2.805);
  ...
  String uvString = String(IndexUV);
  String macString = String(mac);
  String tempString = String(temp);
  String gasString = String(CO2);
  ...
  // Check UV levels and set colors
  if (IndexUV > 8) {
    redUV = 0;
    greenUV = 255;
  } else if (IndexUV >= 5) {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    sprintf(datosUV, "https://ecox.credisur-peru.com//registros?descripcion=IndiceUV&valor=%s&unidad=x/&mac=%s", uvString.c_str(), macString.c_str());
    http.begin(datosUV);
    ...
    if (codigo_respuestaUV > 0) {
      ...
      if (codigo_respuestaTemp > 0) {
        ...
        sprintf(datosGas, "https://ecox.credisur-peru.com//registros?descripcion=Gas&valor=%s&unidad=PPM&mac=%s", gasString.c_str(), macString.c_str());
        http.begin(datosGas);
        http.addHeader("Content-Type", "application/x-www-form-urlencoded");
        ...
        if (codigo_respuestaGas > 0) {
          ...
          sprintf(datosHum, "https://ecox.credisur-peru.com//registros?descripcion=Humedad&valor=%s&unidad=RH&mac=%s", humString.c_str(), macString.c_str());
          http.begin(datosHum);
          http.addHeader("Content-Type", "application/x-www-form-urlencoded");
